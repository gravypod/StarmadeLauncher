...
public class VersionManager {
	...
	public void downloadUpdate(final boolean backup, Component component) throws IOException {
	
		final String buildURL = mirror + desanatizeFile(latestBuild());
		final URL website = new URL(buildURL);
		final ReadableByteChannel rbc = Channels.newChannel(new ProgressMonitorInputStream(component, "Downloading Update", website.openStream()));
		final int fileSize = fileSizes.get(latestBuild());
		File latestVersion = new File(starmadeDirectory, "starmade-latest.zip");
		final FileOutputStream fos = new FileOutputStream(latestVersion);
		fos.getChannel().transferFrom(rbc, 0, Long.MAX_VALUE);
		fos.close();
		rbc.close();
		
		if (isInstalled() && backup) {
			final File f = new File(starmadeDirectory, new SimpleDateFormat("'backup-'MM-dd hh-mm-ss-SS'.zip'").format(new Date()));
			if (!f.exists()) {
				f.createNewFile();
			}
		...
		if (latestVersion.length() != fileSize) {
			throw new IOException("File downloaded is the incorrect size!");
		}
		
		ZipUtils.extract(latestVersion, starmadeDirectory);
	}
